...
const CTAs = ({ isProduct = false }) => {
    const { t } = useTranslation();
    return (
        <section className="cta">
            <div className="container">
                <TextAnimation delay={0.1} start="top bottom">
                    <h3 dangerouslySetInnerHTML={{ __html: replaceText(isProduct ? t("cta.description_product") : t("cta.description")) }} />
                </TextAnimation>
                {isProduct ? (
                    <Link href={t("cta.btn_url_product")} className="btn btn-primary">{t("cta.btn_text_product")}</Link>
                ) : (
